...
router.get('/delThumb',function (req,res) {
    var filePath = path.resolve('D:\\VueWorkSpace\\datacenter1225\\data-center-demo - 1126\\public');//将一个路径或路径片段解析为绝对路径
    //读取public文件夹
 ...
 const{username,useremail,password,type} = req.body
  ...
  UserModel.findOne({username,useremail},function (err,user) {
    ...
    if(user){
      ...
      //保存到数据库
      new UserModel({username,useremail,type,password:md5(password)}).save(function (err,user) {
...
router.post('/login',function (req,res) {
  ...
  UserModel.findOne({username,useremail,password:md5(password)},filter,function (err,user) {
    if(user){//登录成功
       ...
       // console.log(req.session.userid)
        console.log('capthca=',req.cookies.captcha)
    }else{//登录失败
        console.log('capthca=',req.cookies.captcha)
      res.send({code:1,msg:req.cookies.captcha})
...
router.get('/uploadHeader', function (req, res) {
    const headerImg=req.query.headerImg
    const userId=req.query.userId
    UserModel.findByIdAndUpdate({_id:userId}, {
        '$set': {headerImg:headerImg}
    },function (err,user) {
        if(!err){
